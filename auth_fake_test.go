...
// the setup methods needed by auth tests.
...
func (f *fakeSessionStore) CreateSession(_ context.Context, s *Session) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	cp := *s
...
func (f *fakeSessionStore) CreateAPIKey(_ context.Context, k *Key) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	cp := *k
	if cp.ID == 0 {
